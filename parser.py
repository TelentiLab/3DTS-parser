...
def parse_scores(filename: str, total_lines: int = 0) -> None:
    start_time = time.time()
    counter = 0
    skipped_lines = []
    with open('./formatted_scores.tsv', 'w') as file_out:
        with open(filename, 'r') as file:
            for line in file:
                ...
                try:
                    ...
                    skipped_lines.append(line)
                    continue
                file_out.write(f'{key}\t{post_global_synonymous_rate}\n')
                if total_lines:
                    counter += 1
                    if counter % (total_lines // 1000) == 0:
                        ...
                        print(f'reading score file({100 * ratio:.2f}%), estimated time left: {time_left}')
        end_time = time.time()
        print(f'# of lines skipped due to parsing error: {len(skipped_lines)}')
        print(f'formatted score created, time spent: {end_time - start_time}')
...
def process_loci_file(filename: str, total_lines: int = 0):
    ...
    counter = 0
    skipped_lines = []
    print('start processing loci file...')
            ...
            for line in file:
                ...
                if total_lines:
                    ...
                    if counter % (total_lines // 5000) == 0:
                        ratio = counter / total_lines
                        time_left = datetime.timedelta(seconds=(time.time() - start_time) * (1 - ratio) / ratio)
                        print(f'writing result({100 * ratio:.2f}%), estimated time left: {time_left}')
...
parse_scores(_score_file_path, SCORE_LINES)
